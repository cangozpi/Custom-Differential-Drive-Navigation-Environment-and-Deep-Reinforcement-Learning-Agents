...
def main():
    ...
    config_path = 'rl/config/sb3_configs/config_sb3_PPO_DiffDriveEnv.yaml'
    ...
    if mode == "train":
        ...
        model = PPO(
            "MlpPolicy", 
            env, 
            learning_rate= env.config['lr'],
            policy_kwargs={
                # 'activation_fn':torch.nn.LeakyReLU,
                'net_arch':{
                    # 'pi': [64, 32], 'vf': [16, 8]
                    'pi': env.config['actor_hidden_dims'], 'vf': env.config['critic_hidden_dims']
                    }
            },
            batch_size= env.config['batch_size'],
            n_steps= env.config['update_every'],
            n_epochs= env.config['num_updates'],
            gamma= env.config['gamma'],
            verbose=int(env.config['verbose']), 
            tensorboard_log="logs_tensorboard/sb3",
            seed= env.config['seed']
        )

        model.learn(total_timesteps= env.config['total_timesteps'], \
             tb_log_name="sb3_PPO DiffDrive_Env"+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S"), \
                 reset_num_timesteps=False, log_interval=1, progress_bar=True)
        model.save("sb3_PPO_DiffDrive_Env_model")
    ...
    if mode == "test":
        ...
        model = PPO.load("sb3_PPO_DiffDrive_Env_model", env=env) # HER requires env passed in
