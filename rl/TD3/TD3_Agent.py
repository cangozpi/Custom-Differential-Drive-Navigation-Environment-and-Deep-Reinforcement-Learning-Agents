...
class TD3_Agent(): #TODO: make this extend a baseclass (ABC) of Agent and call its super().__init__()
    """
    Refer to https://spinningup.openai.com/en/latest/algorithms/td3.html for implementation details.
    """
    def __init__(self, obs_dim, action_dim, actor_hidden_dims, critic_hidden_dims, actor_lr, critic_lr, initial_epsilon, epsilon_decay, min_epsilon, act_noise, target_noise, clip_noise_range, gamma, tau, max_action, policy_update_delay=2, logger=None, log_full_detail=False):
        ...
        # TD3 learns 2 Q functions isntead of one (hence "Twin")
        self.critic1 = Critic(obs_dim, action_dim, critic_hidden_dims)
        self.critic2 = Critic(obs_dim, action_dim, critic_hidden_dims)

        self.actor_target = Actor(obs_dim, action_dim, actor_hidden_dims, max_action)
        self.critic_target1 = Critic(obs_dim, action_dim, actor_hidden_dims)
        self.critic_target2 = Critic(obs_dim, action_dim, actor_hidden_dims)
        ...
        for actor_target_p, critic_target1_p, critic_target2_p in zip(self.actor_target.parameters(), self.critic_target1.parameters(), self.critic_target2.parameters()):
            actor_target_p.requires_grad = False
            critic_target1_p.requires_grad = False
            critic_target2_p.requires_grad = False

        self.optim_actor = torch.optim.Adam(self.actor.parameters(), actor_lr)
        self.optim_critic1 = torch.optim.Adam(self.critic1.parameters(), critic_lr)
        self.optim_critic2 = torch.optim.Adam(self.critic2.parameters(), critic_lr)
    ...
    def choose_action(self, obs):
        ...
        # During training add noise to the action (exploration)
        if self.mode == "train":
    ...
    def update(self, state_batch, action_batch, reward_batch, next_state_batch, terminal_batch):
        ...
        with torch.no_grad():
            ...
            # Find smallest Q values estimate for next state
            q_next_pred1_batch = self.critic_target1(next_state_batch, clipped_noise_action)
            q_next_pred2_batch = self.critic_target2(next_state_batch, clipped_noise_action)
            smallest_q_pred_batch = torch.minimum(q_next_pred1_batch, q_next_pred2_batch)

            Q_targets = reward_batch + \
                ((1 - terminal_batch.int()) * self.gamma * smallest_q_pred_batch)

        # Update Q functions (critic1 & critic2)
        Q_value_preds_1 = self.critic1(state_batch, action_batch)
        Q_value_preds_2 = self.critic2(state_batch, action_batch)

        critic1_loss = self.critic_loss_func(Q_value_preds_1, Q_targets)
        critic2_loss = self.critic_loss_func(Q_value_preds_2, Q_targets)
        ...
        log_gradients_in_model(self.critic1, self.logger, self._n_updates, "Critic1", self.log_full_detail)
        log_gradients_in_model(self.critic2, self.logger, self._n_updates, "Critic2", self.log_full_detail)
        log_training_losses(critic1_loss.cpu().detach(), self.logger, self._n_updates, "Critic1")
        log_training_losses(critic2_loss.cpu().detach(), self.logger, self._n_updates, "Critic2")

        # ------------------------------------------- Delayed Policy Update (hence "Delayed"):
        if self._n_updates % self.policy_update_delay == 0:
            ...
            for critic1_p in self.critic1.parameters():
                critic1_p.requires_grad = False

            actor_loss = - torch.mean(self.critic1(state_batch, self.actor(state_batch)))
            ...
            log_gradients_in_model(self.actor, self.logger, self._n_updates // self.policy_update_delay, "Actor", self.log_full_detail)
            ...
            for critic1_p in self.critic1.parameters():
                critic1_p.requires_grad = True

            # Update target networks with polyak averaging ("Delayed")
    ...
    def save_model(self):
        ...
        torch.save(
            ...
            'TD3_actor_target.pkl'
        )
        torch.save(
            self.critic_target1.state_dict(),
            'TD3_critic_target1.pkl'
        )
        torch.save(
            self.critic_target2.state_dict(),
            'TD3_critic_target2.pkl'
        )
    ...
    def load_model(self):
        ...
        self.actor.load_state_dict(
            torch.load('TD3_actor.pkl')
        )
        self.critic1.load_state_dict(
            torch.load('TD3_critic1.pkl')
        )
        self.critic2.load_state_dict(
            torch.load('TD3_critic2.pkl')
        )
        # Load target networks too
        self.actor_target.load_state_dict(
            torch.load('TD3_actor_target.pkl')
        )
        self.critic_target1.load_state_dict(
            torch.load('TD3_critic_target1.pkl')
        )
        self.critic_target2.load_state_dict(
            torch.load('TD3_critic_target2.pkl')
        )
