...
def main():
    ...
    config_path = 'rl/config/config_TD3_PendulumEnv.yaml'
...
def train_agent(env):
    ...
    log_dir, run_name = "logs_tensorboard/", "pendulum my_TD3 agent training_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
def test_agent(env):
    # Initialize Tensorboard
    log_dir, run_name = "logs_tensorboard/", "pendulum my_TD3 agent testing_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    tb_summaryWriter = SummaryWriter(log_dir + run_name)
