...
class DiffDrive_Env(Env):
    ...
    def reset(self, target_x=None, target_y=None):
        ...
        if (target_x is not None) and (target_y is not None):
            ...
            # self.target_x = 1.5
            # self.target_y = 1.5
            self.target_x = ((np.random.rand(1) * 6.0) - 3.0).item() # in range [-3, 3]
            self.target_y = ((np.random.rand(1) * 6.0) - 3.0).item() # in range [-3, 3]
    ...
    def step(self, action):
        ...
        if self.use_diff_drive_kinematics:
            ...
            self.theta = self.theta % (np.pi * 2) # in range [0, 2*pi]
        ...
        reward = get_reward(observation, target_state, distance_threshold=self._distance_threshold)
